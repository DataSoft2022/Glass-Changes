          ...
          if(frm.doc.work_order && frm.doc.__unsaved == 1) {
              ...
              callback: function(r) {
                  if(r.message) {
                    ...
                    frappe.show_progress("Operation", 0, frm.doc.items.length, "Organizing Operations");
                    for(let i = 0; i < frm.doc.items.length; i++) {
                        frm.doc.items[i].operation = resp.required_items[i].operation;
                        frappe.show_progress("Operation", i, frm.doc.items.length, "Organizing Operations");
                    }
                    frappe.show_progress("Operation", frm.doc.items.length, frm.doc.items.length, "Organizing Operations");
